...
void estimate(float thetam, float dpsi, float* psi, float* dtheta) {
    float dt = delta_t();
    (*dtheta) = (thetam - thetaprec) / dt;
    thetaprec = thetam;
...
float controleur(float thetam, float psi, float dtheta, float dpsi) {
    return (thetam*0.67 + psi*19.9053 + dtheta*1.0747 + dpsi*1.9614);
}
